...
def model_dsw(data_dir, filename):
    ...
    return results
def pre_process_satellite(data_dir=None, sky="day"):
    if sky == "night":
        timeofday = "night"
    else:
        timeofday = "day"
    Sat_dir = Sat_preprocess(data_dir, sat='GOES17')
    sat = pd.read_hdf(
        os.path.join(data_dir+Sat_dir, "GOES17_day_radiance_satellite_{}.h5".format(sky)),
        'df'
    )
    sat['COD'] = np.nan
    Sun_Zen = sat['Sun_Zen'][0]
    TPW = sat['TPW'][0]
    local_zen = sat['Local_Zen'][0]
    rela_azi = sat['rela_azi'][0]
    COD_guess = 10.0
    learning_rate = 0.01
    r = 1
    ...
    for i in range(sat_ref.shape[0]):
        while (r > 0.01):
            ...
            COD_guess = update_cod_adagrad(df_channel_ref, sat_ref.iloc[i], COD_guess, learning_rate)
            # calculate the difference between the model and the satellite
            del df_channel_ref
        print('row',i,'COD',COD_guess)
        sat['COD'].iloc[i] = COD_guess
    return sat
...
if __name__ == "__main__":
    ...
    for sky in ["day"]:
        sat = pre_process_satellite(data_dir=data_dir, sky=sky)
        sat.to_csv("Result_GOES_{}_radiance_satellite.csv".format(sky), index=False)
