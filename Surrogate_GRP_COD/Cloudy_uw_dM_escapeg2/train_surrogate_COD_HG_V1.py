"""
Train cloudy dM g2-escape FY4A AGRI-channel surrogate V1.
...
DEFAULT_CSV = SCRIPT_DIR / "preprocessed_cloudy_uw_dM_escapeg2.csv"
CHANNELS = ["C01", "C02", "C03", "C05", "C06"]
BASE_FEATURES = ["log1p_COD", "tpw", "cos_th0"]
MODEL_NAME = "SWRTM_cloudy_uw_channel_dM_escapeg2_V1.pkl"
INTERP_MODEL_NAME = "SWRTM_cloudy_uw_channel_dM_escapeg2_interp_V1.pkl"
OUTPUT_TAG = "channel_dM_escapeg2"
TITLE = "Cloudy dM g2-Escape"
RANDOM_STATE = 42
...
def build_interpolator_bundle(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "metadata": {
            "description": "Cloudy dM g2-escape forward surrogate: regular-grid interpolation of the RTM LUT",
...
def plot_scatter(y_true: np.ndarray, y_pred: np.ndarray, out_path: Path) -> None:
    ...
    fig.suptitle(f"{TITLE} Independent Channel Surrogates V1", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train cloudy dM g2-escape independent FY4A AGRI-channel surrogates V1.")
    ...
    metrics.to_csv(out_dir / f"metrics_{OUTPUT_TAG}_V1.csv")
    importance_df = collect_feature_importance(models, feature_map, df.iloc[idx_test])
    importance_df.to_csv(out_dir / f"feature_importance_{OUTPUT_TAG}_V1.csv", index=False)
    plot_scatter(y_test, y_pred, out_dir / f"validation_scatter_{OUTPUT_TAG}_V1.png")
    plot_feature_importance(importance_df, out_dir / f"feature_importance_{OUTPUT_TAG}_V1.png")
    ...
    bundle = {
        ...
        "target_columns": CHANNELS,
        "metadata": {
            "description": "Cloudy dM g2-escape forward surrogate: independent channel models using log1p(COD), TPW, cos(th0), and same-channel albedo",
    ...
    interp_metrics.to_csv(out_dir / f"metrics_{OUTPUT_TAG}_interp_V1.csv")
