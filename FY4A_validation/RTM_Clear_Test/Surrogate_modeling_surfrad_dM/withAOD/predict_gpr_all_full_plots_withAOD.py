"""
predict_gpr_all_full_plots_withAOD.py

Generates the pro-style overall validation scatter plots (GHI only) and seasonal variance plots
using the site-AOD dM V6 GHI prediction CSV files in this `withAOD` folder.
"""
...
def build_pro_scatter_axes(ax, obs, pred, xlabel, ylabel, title, color="#c8e3eb", marker_label='(a)'):
    ...
    ax.plot([vmin, vmax], [vmin, vmax], color='k', linestyle='--', linewidth=1.1, alpha=0.85,zorder=4)
    ...
    ax.set_xlabel(xlabel, fontsize=font_size-1, fontweight='bold')
    ax.set_ylabel(ylabel, fontsize=font_size-1, fontweight='bold')
...
if all_predicted:
    ...
    plt.tight_layout()
    #fig.suptitle('dM V6 GHI Surrogate Performance on FY4A with Site AOD (Mar-Oct)', y=1.02, fontweight='bold', fontsize=14)
    ...
    g_dw.fig.suptitle('Seasonal Variance of dM V6 GHI Predictions by Site with Site AOD', y=1.02, fontsize=16, fontweight='bold')
