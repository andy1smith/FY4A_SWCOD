"""
predict_gpr_all_full_plots_withoutAOD.py

Generates the pro-style overall validation scatter plots (GHI only) and seasonal variance plots
using the without-site-AOD dM V6 GHI prediction CSV files in this `withoutAOD` folder.
"""
...
def build_pro_scatter_axes(ax, obs, pred, xlabel, ylabel, title, color="#c8e3eb", marker_label='(a)'):
    ...
    ax_res.set_xlabel('Residual', fontsize=font_size - 4, fontweight='bold')
...
if all_predicted:
    ...
    g_dw.fig.suptitle('Seasonal Variance of dM V6 GHI Predictions by Site without Site AOD', y=1.02, fontsize=16, fontweight='bold')
