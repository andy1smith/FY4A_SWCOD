...
def preprocess_clearsky_periods(site, lat, lon, alt):
    """
    ...
    df = pd.read_hdf('data/surfrad/{}_ground.h5'.format(site), 'df')
    ...
    for i in range(i_window, df.shape[0], i_window):
        ...
        i_end = i + 1
        ghi = df.iloc[i_start:i_end]['ghi'].values
    ...
    # export data
    df.to_hdf("data/surfrad/{}_day.h5".format(site), "df", mode="w")
    df[df['clearsky'] == True].to_hdf('data/surfrad/{}_clear.h5'.format(site), 'df', mode="w")
    df[df['clearsky'] == False].to_hdf('data/surfrad/{}_cloudy.h5'.format(site), 'df', mode="w")
    print(site, df.shape, df.index[0], df.index[-1])
...
if __name__ == "__main__":
    ...
    for site, lat, lon, alt in sites:
        ...
        preprocess_clearsky_periods(site, lat, lon, alt)
